...
var redTurn = true
turn.innerText = 'Player One'
var running = true;

squares.forEach(square => {
    square.addEventListener('click', () => {
        if(!isEmpty(square) && running) {
            if(redTurn) {
                square.classList.add('player-one')
                if(checkWin('player-one')) {
                    alert('Player One Wins!!!')
                    turn.innerText = 'Player One Won!!!'
                    running = false
                } else {
                    redTurn = false
                    turn.innerText = 'Player Two'
                }
                
            } else {
                square.classList.add('player-two')
                if(checkWin('player-two')) {
                    alert('Player Two Wins!!!')
                    turn.innerText = 'Player Two Won!!!'
                    running = false
                } else {
                    redTurn = true
                    turn.innerText = 'Player One'
